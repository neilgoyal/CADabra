...
response = create_text_to_cad.sync(
    client=client,
    output_format=FileExportFormat.kcl,
    body=TextToCadCreateBody(
        prompt="""
        1.⁠ ⁠Body: A 65 mm diameter, 120 mm tall cylindrical body with vertical indentations for grip, a recessed label area, and horizontal ridges for added stability.
        """
    ),
)
...
if result.status == ApiCallStatus.FAILED:
    ...
    with open("text-to-cad-output.stl", "w", encoding="utf-8") as output_file:
        output_file.write(final_result.decode("utf-8"))
        print(f"Saved output to {output_file.name}")
    with open("text-to-cad-output.gltf", "w", encoding="utf-8") as output_file:
        output_file.write(final_result.decode("utf-8"))
        print(f"Saved output to {output_file.name}")
    with open("text-to-cad-output.glb", "w", encoding="utf-8") as output_file:
        output_file.write(final_result.decode("utf-8"))
        print(f"Saved output to {output_file.name}")
    with open("text-to-cad-output.kcl", "w", encoding="utf-8") as output_file:
        output_file.write(final_result.decode("utf-8"))
        print(f"Saved output to {output_file.name}")